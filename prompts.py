...
2. **ask_pricing** (Info)
    - Slots:
        ...
        - subscription_type (Allowed: [single_entry, 10_entries, monthly, annual, seasonal])
...
7. **modify_booking** (Management)
    - Slots:
        - booking_type (e.g., course, wellness)
        - old_date (e.g., today, tomorrow, Monday, 2023-10-25)
        - new_date (e.g., today, tomorrow, Monday, 2023-10-25)
        - new_time (e.g., 10:00, 15:30, evening)
    - Description: User wants to change an existing reservation.

8. **report_lost_item** (Assistance)
...
DM_ACTION_PROMPT = """You are given the dialogue state for the current turn.
...
Decide the Next Best Action based on the State and (optional) DB Result.

### Available Actions:
- `request_info(slot)`: Missing required slot.
- `query_db(intent)`: All slots are present, need to check DB for availability/price/info.
- `confirm_booking`: DB check was SUCCESSFUL, confirm to user.
- `deny_request(reason)`: DB check FAILED (e.g., full, closed, not found). Explain why.

### input Context:
State: {dialogue_state}
Previous DB Result: {db_result}
"""
